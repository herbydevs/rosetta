<template>
  <div class="thread-card" @click="goToThread">
    <h1 class="thread title">{{ thread.title }}</h1>
    <div class="thread-info">
      <p class="thread description">{{ thread.description }}</p>
      <p class="comments">💬 {{ thread.comments }}</p>
      <p class="created-at">Created at: {{ thread.createdAt }}</p>
      <p class="upvotes">👍 {{ thread.upvotes }}</p>
    </div>
  </div>
</template>

<script>
export default {
  name: 'CommentCard',
  props: {
    thread: {
      type: Object,
      default: () => ({
        id: 1,
        title: "Loading...",
        description: "",
        comments: 0,
        createdAt: "",
        upvotes: 0
      })
    }
  },
  methods: {
    goToThread() {
      this.$router.push({ name: 'thread', params: { id: this.thread.id } })
    }
  }
}
</script>


<style scoped>
.thread-card {
  border: 1px solid #ccc;
  border-radius: 8px;
  padding: 16px;
  margin: 16px 0;
}
.thread {
  font-size: 1.5em;
  margin-bottom: 8px;
}
.thread-info {
  display: flex;
  justify-content: space-between;
  font-size: 0.9em;
  color: #555;
}
.thread-info p {
  margin: 0;
}
.thread-info .comments,
.thread-info .upvotes {
  display: flex;
  align-items: center;
}
.thread-info .created-at {
  color: #888;
}
.thread-info .description {
  flex: 1;
  margin-right: 16px;
}
.thread-info .description::after {
  content: '';
  display: block;
  height: 1px;
  background: #eee;
  margin-top: 8px;
}
.thread-info .description {
  flex: 1;
  margin-right: 16px;
}
.thread-info .description::after {
  content: '';
  display: block;
  height: 1px;
  background: #eee;
  margin-top: 8px;
}
.thread-info .upvotes {
  color: #f39c12;
}
.thread-info .comments {
  color: #3498db;
}
.thread-info .created-at {
  color: #7f8c8d;
}
.thread-info .upvotes {
  color: #e74c3c;
}
</style>